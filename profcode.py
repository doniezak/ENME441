...
# Stepper class
#
# Code allows multiple motors to run in parallel
...
class Stepper:
    ...
    def __step(self, dir):
        ...
        # The next 3 lines use bitwise operators to change the 4 bits
        # within Stepper.shifter_outputs for the current Stepper object,
        # without changing any bits associated with other Steppers. This
        # is critical for allowing multiple motors to be operated in parallel
        # from a single Shifter object (which may be involve a single
        # physical shift register or multiple chained shift registers).
        #
        # Create bit mask with 0000 for current motor and 1111 for all other motors:
        mask = ~(0b1111<<self.shifter_bit_start)
        # Apply the mask to clear bits for current motor:
        Stepper.shifter_outputs &= mask   
        # Set new bits for current motor:
    ...
    def __rotate(self, delta):
        with self.lock:            # wait until the lock is available (acquire & release at end)
